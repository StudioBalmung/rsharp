...
static void mkdirp(const char *path) {
    ...
    for (char *p = t+1; *p; p++) if (*p=='/') { *p=0; PLATFORM_MKDIR(t,0755); *p='/'; }
    PLATFORM_MKDIR(t, 0755);
}
...
int main(int argc, char **argv) {
    g_color = PLATFORM_ISATTY(STDERR_FILENO);

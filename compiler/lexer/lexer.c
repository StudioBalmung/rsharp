...
static Token error_tok(Lexer *l, SrcPos s, const char *msg) {
    ...
    if (b) { memcpy(b,msg,n+1); t.str.ptr=b; } else { t.str.ptr=""; }
    t.str.len=n; return t;
}
...
static Token lex_string(Lexer *l, SrcPos start) {
    size_t cap = 4096;
    char *buf=arena_alloc(l->arena,cap,1); size_t bi=0;
    if (!buf) return error_tok(l,start,"out of memory");
    while (!at_end(l)) {
        if (bi + 2 >= cap) {
            size_t new_cap = cap * 2;
            char *new_buf = arena_alloc(l->arena, new_cap, 1);
            if (!new_buf) return error_tok(l,start,"out of memory in string literal");
            memcpy(new_buf, buf, bi);
            buf = new_buf;
            cap = new_cap;
        }
        char c=advance(l);
...
static Token lex_raw_string(Lexer *l, SrcPos start) {
    ...
    while (!at_end(l)) {
        if (bi + hashes + 2 >= cap) {
            size_t new_cap = cap * 2;
            char *new_buf = arena_alloc(l->arena, new_cap, 1);
            if (!new_buf) return error_tok(l,start,"out of memory in raw string");
            memcpy(new_buf, buf, bi);
            buf = new_buf;
            cap = new_cap;
        }
...
Token lexer_next(Lexer *l) {
    ...
    if (isalpha(c)||c=='_') {
        ...
        if (kind==TOK_IDENT) {
            ...
            if (in) { memcpy(in,s,len); in[len]='\0'; t.str.ptr=in; } else { t.str.ptr=""; }
            t.str.len=len;

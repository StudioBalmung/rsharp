...
static Expr *parse_primary(Parser *p) {
    ...
    if (match_tok(p, TOK_NULL_LIT)) {
        e->kind = EXPR_INT_LIT; /* null represented as 0 in AST */
        e->int_val = 0;
        e->span = p->previous.span;
        return e;
    }
